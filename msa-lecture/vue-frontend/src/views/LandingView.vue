<template>
  <div class="landing">
    ...
    <main class="page-container landing-container">
      <!-- 검색 -->
          ...
          <ol class="ranking-list">
            ...
            >
              <router-link to="/login" class="ranking-link">
                <span class="ranking-num" :class="{ top: i < 3 }">{{
                  i + 1
                }}</span>
                <img
                  :src="course.thumbSrc"
                ...
                <div class="ranking-info">
                  <span class="badge" :class="course.badgeClass">{{
                    course.category
                  }}</span>
      ...
      <section class="landing-section">
        <div class="section-header">
          <div>
            <span class="section-kicker">OPENING SOON</span>
          ...
          <div class="upcoming-grid">
            ...
            >
              ...
              <div class="upcoming-body">
                <span class="badge" :class="show.badgeClass">{{
                  show.category
                }}</span>
      ...
      <section class="landing-section">
        <div class="section-header">
          <div>
            <span class="section-kicker">LIMITED DEALS</span>
        ...
        <div class="card-surface section-body">
          <div class="discount-grid">
            <router-link
              v-for="ticket in discountedTickets"
              :key="ticket.id"
              to="/login"
              class="discount-card"
            >
              <div class="discount-thumb">
                ...
                <span class="discount-badge"
                  >{{ ticket.discountPercent }}%</span
                >
              </div>
              <div class="discount-body">
                <span class="badge" :class="ticket.badgeClass">{{
                  ticket.category
                }}</span>
                <h3 class="discount-title">{{ ticket.title }}</h3>
                <div class="discount-price-row">
                  <span class="discount-original"
                    >₩{{ ticket.originalPrice.toLocaleString() }}</span
                  >
                  <span class="discount-final"
                    >₩{{ ticket.discountPrice.toLocaleString() }}</span
                  >
                </div>
              </div>
            </router-link>
      ...
      <section class="landing-section">
        ...
        <div class="card-surface section-body">
          ...
          <div v-if="genreFilteredCourses.length" class="course-grid">
            <div
              v-for="course in genreFilteredCourses"
              :key="course.id"
      ...
      <section class="cta-section card-surface">
        <span class="section-kicker cta-kicker">JOIN TICKETNEXUS</span>
        <h2>지금 바로 시작하세요</h2>
...
const featuredCourses = [
  ...
  {
    id: 5,
    title: "클래식 갈라 콘서트 <현의 울림>",
    category: "클래식",
    instructor: "필하모닉프로덕션",
    price: "₩59,000",
    thumbSrc: classicImg,
    thumbBg: "thumb-purple",
    badgeClass: "badge-purple",
  },
  {
    id: 6,
    title: "2026 썸머 뮤직 페스티벌",
    category: "페스티벌",
    instructor: "빅웨이브페스티벌컴퍼니",
    price: "₩75,000",
    thumbSrc: festivalImg,
    thumbBg: "thumb-pink",
...
function prevBanner() {
  activeBanner.value =
    (activeBanner.value - 1 + banners.length) % banners.length;
}
...
const upcomingShows = [
  {
    id: "u1",
    title: "클래식 갈라 콘서트 <현의 울림> 시즌2",
    category: "클래식",
    openDate: "2026.08.20 10:00",
    dday: "D-13",
    thumbSrc: classicImg,
    badgeClass: "badge-purple",
  },
  {
    id: "u2",
    title: "연극 <겨울, 그리고 봄> 앙코르",
    category: "연극",
    openDate: "2026.08.16 18:00",
    dday: "D-9",
    thumbSrc: act2Img,
    badgeClass: "badge-blue",
  },
  {
    id: "u3",
    title: "2026 드림 콘서트 <봄의 왈츠> 지방투어",
    category: "콘서트",
    openDate: "2026.08.12 12:00",
    dday: "D-5",
    thumbSrc: concertImg,
    badgeClass: "badge-teal",
  },
...
const discountedTickets = [
  ...
  ...t,
  discountPrice:
    Math.round((t.originalPrice * (1 - t.discountPercent / 100)) / 100) * 100,
}));
...
.banner-teal {
  background: linear-gradient(135deg, #e1f5ee 0%, #eaf9f3 100%);
}
.banner-blue {
  background: linear-gradient(135deg, #e6f1fb 0%, #eef7ff 100%);
}
.banner-pink {
  background: linear-gradient(135deg, #fbeaf0 0%, #fdf1f5 100%);
}
.banner-inner {
  max-width: 1200px;
  height: 100%;
...
.ranking-item:nth-last-child(-n + 2) {
  border-bottom: none;
}
.ranking-link {
  display: flex;
